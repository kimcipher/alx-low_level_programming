...
6. [Real programmers can write assembly code in any language](./101-hello_alx.asm) : A 64-bit program in assembly that prints `Hello, alx` followed by a new line.
	- You are only allowed to use `printf` function.
	- You are not allowed to use interrupts.
	- Your program will be compiled using `nasm` and `gcc` this way: `nasm -f elf64 101-hello_alx.asm && gcc -no-pie -std=gnu89 101-hello_alx.o -o hello`
